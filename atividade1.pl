...
Part 1: Write queries to answer the following questions.
    a. Qual ano american_beauty foi lançado? RESPOSTA: movie(american_beauty, Y).
    b. Filmes lançados em 2000. RESPOSTA: movie(M, 2000).
    c. Filmes lançados antes 2000. RESPOSTA: movie(M, Y), Y < 2000.
    d. Filmes lançados depois de 1990. RESPOSTA: movie(M, Y), Y > 1990
    e. Diretores em filmes que Scarlett Johansson aparece. RESPOSTA: actress(M, scarlett_johansson, _), director(M, D).
    f. Ator ou atriz que também foi diretor. RESPOSTA:  (actor(M, A, _), director(M2, A)); (actress(M, A, _), director(M2, A))
    g. Filme em que John Goodman e Jeff Bridges forma co-estrelas. RESPOSTA: actor(M, john_goodman, _), actor(M, jeff_bridges, _).

Part 2: Adicionar as seguintes regras,
    a. lancado_depois(M, Y) <- filme foi lançado depois de um ano. RESPOSTA: lancado_after(M, Y) :- movie(M, Y1), Y1 > Y
    b. lancado_antes(M, Y) <- filme foi lançado antes de um ano. RESPOSTA: lancado_before(M, Y) :- movie(M, Y1), Y1 < Y.
    c. mesmo_ano(M1, M2) <- <- filme foi lançados no mesmo no. RESPOSTA: same_year(M1, M2) :- movie(M1, Y), movie(M2, Y), M1 \= M2.
    d. co_star(A1, A2) <- Ator ou atriz que for co-estrelas RESPOSTA: co_star(A1, A2) :- (actor(M, A1, _), actor(M, A2, _), A1 \= A2); (actress(M, A1, _), actress(M, A2, _), A1 \= A2); (actor(M, A1, _), actress(M, A2, _), A1 \= A2).
*/
